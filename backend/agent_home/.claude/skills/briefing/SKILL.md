...
2. Call `mcp__guidelines__search_clinical_guidelines` for each major
   ...
   not vague ones (e.g. "diabetes"). If the guidelines search returns nothing
   relevant for a major condition and the web-research skill is available,
   you may search the web for authoritative guidance and cite the source URL
   in the flag/action description — local guidelines remain the primary
   source.
