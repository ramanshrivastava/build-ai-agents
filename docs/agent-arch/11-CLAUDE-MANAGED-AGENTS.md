...
> [!WARNING]
> **Reality check (event-replay) —** Listing session events returns the session's *entire append-only history*, not just what's new. On a reused session, a naive poll loop will happily re-handle last week's `agent.custom_tool_use` events — re-running tools and re-posting results into a conversation that already moved on. This is invisible on request one and guaranteed on request two. The live code defends twice: `_list_event_ids()` snapshots every pre-existing event ID *before* sending the new patient message, and `_wait_for_briefing_json()` keeps a per-run `handled_tool_ids` set so no tool event is dispatched twice. If you build on Managed Agents and skip this, you have written a replay bug, not an agent.
